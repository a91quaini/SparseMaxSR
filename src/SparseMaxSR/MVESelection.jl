...
# Arguments
- `μ::AbstractVector{Float64}`: Expected returns.
...
# Returns
- `selection::AbstractVector{Int}`: Optimal subset of selected asset indices.
"""
...
# Returns
- `selection::AbstractVector{Int}`: Indices of the k selected assets.
"""
...
# Arguments
- `μ::AbstractVector{Float64}`: Expected returns vector of length n.
- `Σ::AbstractMatrix{Float64}`: Covariance matrix (n×n) of asset returns.
- `k::Int`: Number of assets to select (must satisfy 1 ≤ k ≤ n).
- `λ::AbstractVector{Float64}` (optional): Ridge regularization parameters (risk aversion coefficients) of length n. If not provided,
...
function mve_selection_cutting_planes(
    ...
    Σ::AbstractMatrix{Float64},
    k::Int;
    λ::AbstractVector{Float64}=Float64[],
    ...
    model = Model(optimizer_with_attributes(
        ...
        "CPX_PARAM_SCRIND" => 0,     # ← turn off all CPLEX screen output
    ))
